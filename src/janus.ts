...
class Janus extends events.EventEmitter {

    private gateway:Gateway
    private session:Session
    private infoHandle:Handle
    private streamMap:Map<number,Handle>
    ...
    async init() {
        ...
        return new Promise((resolve,reject) => {
            ...
            this.gateway.on('open', async () => {
                this.session = await this.gateway.create()
                this.infoHandle = await this.session.attach('janus.plugin.streaming')
                resolve()
            })
        })
    }
